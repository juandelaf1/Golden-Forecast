...
def get_regression_models():
    models = {
        "Random Forest": RandomForestRegressor(
            n_estimators=300, max_depth=12, min_samples_leaf=15, random_state=42, n_jobs=-1
        ),
    ...
    if XGB_AVAILABLE:
        models["XGBoost"] = XGBRegressor(
            n_estimators=300, max_depth=5, learning_rate=0.05, random_state=42, verbosity=0, n_jobs=-1
        )
    return models
...
def get_best_model_for_target(target_name: str):
    ...
    return best_configs.get(
        target_name,
        RandomForestRegressor(n_estimators=300, max_depth=12, min_samples_leaf=15, random_state=42, n_jobs=-1),
    )
...
def run_all_targets(
    ...
    for target in TARGET_SPECS.keys():
        ...
        r = results[target]
        print(
            f"  Mejor: {r['best_model_name']} | R²={r['eval_results'].loc[r['eval_results']['R²'].idxmax(), 'R²']:.4f} | IC={r['eval_results'].loc[r['eval_results']['R²'].idxmax(), 'IC (Pearson)']:.4f}"
        )
...
if __name__ == "__main__":
    ...
    if isinstance(r, dict) and "eval_results" in r:
        ...
        for t, res in r.items():
            ...
            print(
                f"{t:20s} | R²={best['R²_mean']:>7.4f}±{best['R²_std']:.4f} | IC={best['IC_mean']:>6.3f}±{best['IC_std']:.3f} | RankIC={best['RankIC_mean']:>6.3f}±{best['RankIC_std']:.3f} | Best={best['Modelo']}"
            )
