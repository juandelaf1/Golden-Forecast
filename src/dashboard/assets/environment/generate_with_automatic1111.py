...
def main(args):
    ...
    manifest = json.load(open("assets/environment/generation_manifest.json", "r", encoding="utf-8"))
    prompt = open("assets/environment/prompt.txt", "r", encoding="utf-8").read().strip()
    negative = open("assets/environment/negative.txt", "r", encoding="utf-8").read().strip()
    ...
    for v in manifest["variants"]:
        ...
        try:
            r = requests.post(base + "/sdapi/v1/txt2img", json=payload, headers=headers, timeout=600)
            r.raise_for_status()
            data = r.json()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base_url", default="http://127.0.0.1:7860", help="Automatic1111 base URL")
    parser.add_argument("--api_key", default="", help="API key if your instance requires one")
