...
CLEAN_PATH = os.path.join(
    os.path.dirname(__file__), "..", "..", "data", "processed", "gold-clean.csv"
)
...
def load_feature_data() -> pd.DataFrame:
    ...
    merged = features.merge(
        clean[["Date", "gold_close", "dxy_close", "vix_close", "tnx_close"]],
        on="Date",
        how="left",
    )
    ...
    merged["ma_21"] = (
        merged["gold"].rolling(21).mean().fillna(merged["gold"].expanding().mean())
    )
...
def _build_model_results(
    ml_context: dict, data: pd.DataFrame, split_index: int
) -> dict:
    ...
    for entry in eval_data["resultados"]:
        ...
        probas = (
            predictions[name]["probabilities"][split_index:][:n_returns]
            if predictions[name]["probabilities"] is not None
            else None
        )

        is_multiclass = "multiclass" in name
        y_test = (
            y_multi_test[:n_returns] if is_multiclass else y_binary_test[:n_returns]
        )
        ...
        prec = entry.get(
            "precision_test",
            precision_score(y_test, preds, average=avg, zero_division=0),
        )
        rec = entry.get(
            "recall_test", recall_score(y_test, preds, average=avg, zero_division=0)
        )
...
def build_context() -> dict:
    ...
    test_target = (
        ml_context["y_multiclass"].iloc[split_index:].values
        if is_multi_primary
        else ml_context["y_binary"].iloc[split_index:].values
    )
    ...
    change_pct = (
        float((change / float(prev["gold"])) * 100) if float(prev["gold"]) > 0 else 0
    )
