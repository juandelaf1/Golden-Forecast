        ...
        results.append(
            {
                "Modelo": name,
                "Estrategia": round(
                    float(cum_strategy.iloc[-1]) if hasattr(cum_strategy, "iloc") else cum_strategy[-1], 4
                ),
...
def train_experimental_models(X_train, X_test, y_reg_train, y_reg_test, feature_columns):
    ...
    for model_name, model in model_defs.items():
        ...
        for h in HORIZONS:
            ...
            res = _eval_model(
                ...
                X_train[valid_train],
                X_test[valid_test],
                target_train[valid_train].astype(int),
                target_test[valid_test].astype(int),
                h,
            )
    ...
    try:
        ...
        results["voting"] = {
            ...
            "precision": round(
                precision_score(target_test_1d[valid_test_v].astype(int), voting_preds, zero_division=0), 4
            ),
